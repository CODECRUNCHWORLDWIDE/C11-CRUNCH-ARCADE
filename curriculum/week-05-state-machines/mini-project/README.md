...
- **Apply the FSM to an enemy.** This is essentially the [Challenge 1](../challenges/challenge-01-enemy-ai-fsm.md) — `patrol`, `chase`, `attack`, `return`. Add one enemy to the mini-project. Each enemy is its own `Character` instance, each in its own state. The pattern is the same.
...
- This week's [Lecture 1](../lecture-notes/01-hand-rolled-fsm.md) and [Lecture 2](../lecture-notes/02-state-transitions-and-substates.md).
- The week's [exercises](../exercises/) — copy from them.
- The week's [challenge](../challenges/challenge-01-enemy-ai-fsm.md) — the enemy FSM is the natural stretch after the player.
